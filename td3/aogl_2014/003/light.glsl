...
uniform float ShadowNumSamples;
...
void main(void)
{
	...
	vec4 wlightSpacePosition = WorldToShadowmap * vec4(position, 1.0);
	vec3 lightSpacePosition = vec3(wlightSpacePosition/wlightSpacePosition.w);

	if(lightSpacePosition.x < 0 || lightSpacePosition.x > 1 || lightSpacePosition.y < 0 || lightSpacePosition.y > 1)
	{
		Color = vec4(0, 0, 0, 1);
	}
	else
	{
		...
		float visibilityOffset = 1.0 / ShadowNumSamples;
