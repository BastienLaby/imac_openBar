...
public:
    ...
    // Should have n>=1,
    // for n==0 one set with the element zero is generated.
    {
        n_ = n;
        n1_ = (n_ ? n_ - 1 : 0 );
#ifndef SUBSET_LEX_MAX_ARRAY_LEN
    ...
    // Loopless algorithm.
    {
        if ( k_ == 0 )  // only one element ?
        {
            ...
            x_[0]--;  // decrement first element
            x_[++k_] = n1_;  // append element
        }
        else
        {
