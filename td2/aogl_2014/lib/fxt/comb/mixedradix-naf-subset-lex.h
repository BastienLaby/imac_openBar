...
public:
    ...
    void first()
    {
        ...
        // special case: empty set with one element zero
        iset_[0] = 0;
        ni_ = 1;
...
//        j_ = n_;
    ...
    // Loopless algorithm.
    {
        ...
        const ulong lf = iset_[ni_-2];
        const ulong al = a_[lf] - 1;  // decrement left digit
        a_[lf] = al;
        if ( al == 0 )  // move digit one to the right
        {
            a_[lf+1] = 1;
            iset_[ni_-2] = lf + 1;
            --ni_;
    ...
    // Loopless algorithm.
    {
        ...
        const ulong lf = iset_[ni_-2];  // left neighbor

        if ( lf == j - 2 )  // Left neighbor is two-adjacent
        {
            if ( a_[lf] == m1_[lf] )  // Left is nine: simply detach last digit
            {
                --ni_;
                return true;
            }
